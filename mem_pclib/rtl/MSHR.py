...
from pymtl3.stdlib.rtl.registers import RegEnRst, RegRst, Reg
...
#--------------------------------------------------------------------
    ...
    # s.num_id_match_M0 = Wire(BitsCount)
    ...
    def ctrl_signals_logic_M0():
      ...
      # replace with below
      if s.dealloc_req.msg != "r":
        s.val_M0 = s.alloc_req.en or s.dealloc_req.en
      elif s.replay_out_M0:
        s.val_M0 = y
      else:
        s.val_M0 = n
    ...
    s.id_comp = [
      ECompEn(BitsClogEntries)(
        ...
        in1 = s.dealloc_req.msg[0:log_entries],
        en  = s.reg_val_out[i], # val
        out = s.id_match[i],
      ) for i in range(entries)
    ]    
    ...
    def alloc_dealloc_entries_logic():
      ...
      if s.val_M0:
        if s.type_M0 == al:
          ...
          s.replay_in_M0 = y if count > 1 else n
...
#--------------------------------------------------------------------
  ...
  def line_trace(s):
    ...
    msg += f' c[{s.MSHR_entry_count_out}]'
    # msg += ' tgid_m[{:b}]'.format(int(s.tag_index_match))
    # msg += ' id_m[{:b}]'.format(int(s.id_match))
