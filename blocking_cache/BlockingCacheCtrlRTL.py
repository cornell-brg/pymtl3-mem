  ...
  def construct( s, p ):
    ...
    def replay_logic_M0():
      ...
      if s.mem_resp_state_M0.out == IDLE: 
        if s.memresp_val_M0:
          ...
          if s.status.MSHR_type == READ or s.status.MSHR_type == WRITE: 
    ...
    def comb_block_M0(): # logic block for setting output ports
      ...
      if s.state_M0.val: #                                           tag_wben|wdat_mux|addr_mux|memrp_mux|tg_ty|val
        ...
        elif s.state_M0.is_amo:                      s.cs0 = concat( tg_wbenf, b1(1)  , b1(0)  , b1(1)   ,  wr , n )
        else:
    ...
    def tag_array_val_logic_M0():
      ...
      if s.state_M0.val:
        if s.state_M0.is_refill or s.state_M0.is_write_refill or s.state_M0.is_amo:
    ...
    def Asso_data_array_offset_way_M1():
      ...
      if s.state_M1.out.val:
        ...
        elif s.hit_M1 or s.status.cachereq_type_M1 == INIT:
          s.ctrl.way_offset_M1 = s.status.hit_way_M1
        elif s.is_evict_M1:
    ...
    def comb_block_M2(): # comb logic block and setting output ports
      ...
      if s.state_M2.out.val:                                  #  dsize_en|rdata_mux|ostall|memreq_type|memreq|cacheresp
        if s.state_M2.out.is_write_hit_clean:      s.cs2 = concat(   y   ,  b1(0)  ,  n   ,   READ    ,    n ,     n   )
          ...
          elif s.status.cachereq_type_M2>=AMO_ADD: s.cs2 = concat(   n   ,  b1(1)  ,  n   ,   AMO     ,    y ,     n   )
  ...
  def line_trace( s ):
    ...
    if s.state_M1.out.val:
      if s.state_M1.out.is_refill:
        ...
        msg_M1 = "wf"
      elif ~s.hit_M1 and s.status.cachereq_type_M1 != 2:
    ...
    add_msgs = ""
    # add_msgs = f"vla:{s.is_line_valid_M1}"
