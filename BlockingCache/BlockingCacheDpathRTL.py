...
class BlockingCacheDpathRTL (Component):
  ...
  def construct( s, param ):
    ...
    # Proc -> Cache
    s.cachereq_opaque_M0  = InPort(param.BitsOpaque)
    s.cachereq_type_M0    = InPort(param.BitsType)
    s.cachereq_addr_M0    = InPort(param.BitsAddr)
    s.cachereq_data_M0    = InPort(param.BitsData)
    s.cachereq_len_M0     = InPort(param.BitsLen)
    
    # Mem -> Cache
    s.memresp_opaque_Y    = InPort(param.BitsOpaque)
    s.memresp_type_Y      = InPort(param.BitsType)
    s.memresp_data_Y      = InPort(param.BitsCacheline)
    ...
    s.ctrl_bit_rep_wr_M0    = InPort(param.param.BitsAssoclog2)
    ...
    # Signals for multiway param.associativity
    s.tag_match_way_M1      = OutPort(param.param.BitsAssoclog2)
    s.way_offset_M1         = InPort(param.param.BitsAssoclog2)
    s.ctrl_bit_rep_en_M1    = InPort(Bits1)
    s.ctrl_bit_rep_rd_M1    = OutPort(param.param.BitsAssoclog2)
    if param.associativity == 1: # Not necessary for Dmapped cache
      s.tag_match_way_M1  //= param.param.BitsAssoclog2(0)
      s.ctrl_bit_rep_rd_M1//= param.param.BitsAssoclog2(0)
    #---------------------------------------------------------------------------
    # M2 Dpath Signals
    #--------------------------------------------------------------------------
    ...
    s.cachereq_addr_M1    = Wire(param.BitsAddr)
    ...
    def replicator(): # replicates based on word access
      if s.len_M0 == 1: 
        ...
        for i in range(0,param.bitwidth_cacheline,param.bitwidth_data):
          s.rep_out_M0[i:i+param.bitwidth_data] = s.cachereq_data_M0
    ...
    if param.associativity == 1:
      ...
      s.ctrl_bit_rep_M1 = Wire(param.param.BitsAssoclog2) 
      
      # Register File to store the Replacement info
      # Can't store in SRAM since we need to constantly access
      # them for LRU
      s.replacement_bits_M1 = RegisterFile( param.param.BitsAssoclog2, param.nblocks_per_way )
      s.replacement_bits_M1.raddr[0] //= s.cachereq_addr_M1[param.bitwidth_offset:param.bitwidth_index+param.bitwidth_offset]
      s.replacement_bits_M1.rdata[0] //= s.ctrl_bit_rep_M1
      s.replacement_bits_M1.waddr[0] //= s.cachereq_addr_M1[param.bitwidth_offset:param.bitwidth_index+param.bitwidth_offset]
    ...
    s.offset_M1 //= s.cachereq_addr_M1[0:param.bitwidth_offset]
    # Comparator
    if param.associativity == 1:
      @s.update
      def DmappedComparator_M1():
        ...
        if ( s.ctrl_bit_val_rd_M1 ):
          if s.tag_array_rdata_M1[0][0:param.bitwidth_tag] == s.cachereq_addr_M1[param.bitwidth_index+param.bitwidth_offset:param.bitwidth_offset+param.bitwidth_index+param.bitwidth_tag]:
            s.tag_match_M1 = y
    else: # Multiway asso
      s.match_way_M1 = Wire(param.param.BitsAssoclog2)
      ...
      def AssoComparator_M1():
        ...
        for i in range( param.associativity ):
          if ( s.ctrl_bit_val_rd_M1[i] ):
            if s.tag_array_rdata_M1[i][0:param.bitwidth_tag] == s.cachereq_addr_M1[param.bitwidth_index+param.bitwidth_offset:param.bitwidth_offset+param.bitwidth_index+param.bitwidth_tag]:
              s.tag_match_M1 = y
              s.match_way_M1 = param.param.BitsAssoclog2(i) # If not valid, then no comparisons
    ...
    s.evict_addr_M1[param.bitwidth_offset:param.bitwidth_offset+param.bitwidth_index] //= s.cachereq_addr_M1[param.bitwidth_offset:param.bitwidth_offset+param.bitwidth_index]
    ...
    if param.associativity == 1:
      ...
      s.data_array_idx_M1 //= s.cachereq_addr_M1[param.bitwidth_offset:param.bitwidth_index+param.bitwidth_offset]
    else:
      ...
      @s.update
      def choice_calc_M1():
        s.data_array_idx_M1 = Bitsparam.total_num_cachelines(s.cachereq_addr_M1[param.bitwidth_offset:param.bitwidth_index+param.bitwidth_offset]) \
    ...
    s.offset_M2               //= s.cachereq_addr_M2[0:param.bitwidth_offset]
    s.memreq_opaque_M2        //= s.cacheresp_opaque_M2
    s.memreq_addr_M2[param.bitwidth_offset:param.bitwidth_addr] //= s.cachereq_addr_M2[param.bitwidth_offset:param.bitwidth_addr]
