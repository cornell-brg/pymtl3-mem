...
class BlockingCacheCtrlPRTL(Component):
    ...
    def construct(s, dbw=32, ofw=4, BitsAddr='inv', BitsOpaque='inv', BitsType='inv', BitsData='inv', BitsCacheline='inv', BitsIdx='inv', BitsTag='inv', BitsOffset='inv', BitsTagWben='inv', BitsDataWben='inv', BitsRdDataMux='inv', twb=4, dwb=16, rmx2=3):
        ...
        s.state_transition_block = RegRst(Bits3, STATE_GO)(out=s.curr_state, in_=s.next_state)
            ...
            if (s.curr_state == STATE_GO):
                ...
                if (s.is_refill_M0 and (s.memresp_type_M0 != WRITE)):
                    if (s.MSHR_type == WRITE):
                        s.next_state = STATE_REFILL_WRITE
                    else:
                        s.next_state = STATE_GO
                else:
                    s.next_state = STATE_REFILL
                ...
                s.next_state = STATE_REFILL
            elif (s.curr_state == STATE_REFILL_WRITE):
                s.next_state = STATE_GO
            else:
        ...
        s.is_refill_reg_M0 = RegRst(Bits1)(in_=s.memresp_en, out=s.is_refill_M0)

        @s.update
        def cachereq_logic():
            s.cachereq_rdy = ((~ s.stall_M1) and (s.curr_state == STATE_GO) and (s.next_state != STATE_REFILL) and (s.next_state != STATE_EVICT) and (s.curr_state != STATE_REFILL_WRITE))
        ...
        def comb_block_M0():
            ...
            if s.val_M0:
                if s.is_refill_M0:
                    s.cs0 = concat(tg_wbenf, b2(1), b2(1), b1(1), wr, y, n, y)
                elif s.is_write_refill_M0:
                    s.cs0 = concat(tg_wbenf, b2(2), b2(1), b1(1), wr, y, y, y)
                elif s.is_write_hit_clean_M0:
                    s.cs0 = concat(tg_wbenf, b2(0), b2(2), b1(0), wr, y, y, y)
                elif (s.cachereq_type_M0 == INIT):
                    s.cs0 = concat(tg_wbenf, b2(0), b2(0), b1(0), wr, y, n, y)
                elif (s.cachereq_type_M0 == READ):
                    s.cs0 = concat(tg_wbenf, b2(0), b2(0), b1(0), rd, y, n, n)
        ...
        s.val_reg_M1 = RegEnRst(Bits1)(en=s.reg_en_M1, in_=s.val_M0, out=s.val_M1)
        s.is_refill_reg_M1 = RegRst(Bits1)(in_=s.is_refill_M0, out=s.is_refill_M1)
        s.is_write_refill_reg_M1 = RegRst(Bits1)(in_=s.is_write_refill_M0, out=s.is_write_refill_M1)
        s.is_write_hit_clean_reg_M1 = RegRst(Bits1)(in_=s.is_write_hit_clean_M0, out=s.is_write_hit_clean_M1)

        @s.update
        def hit_logic_M1():
            s.hit_M1 = ((s.tag_match_M1 and s.ctrl_bit_val_rd_M1 and (s.cachereq_type_M1 != INIT)) or s.is_write_refill_M1)
            if s.is_write_hit_clean_M1:
                s.hit_M1 = (s.hit_M1 or b1(1))
            s.hit_M2[1] = b1(0)
        s.wben_out = Wire(BitsDataWben)
        s.wben_in = Wire(BitsDataWben)
        s.WbenGen = LShifter(BitsDataWben, clog2(dwb))(in_=s.wben_in, shamt=s.offset_M1, out=s.wben_out)
        ...
        def is_write_hit_clean_M0_logic():
            if (s.cachereq_type_M1 and s.hit_M1 and (~ s.ctrl_bit_dty_rd_M1) and (~ s.is_write_hit_clean_M1) and (~ s.is_write_refill_M1)):
        ...
        def comb_block_M1():
            ...
            if s.val_M1:
                if s.is_refill_M1:
                    s.cs1 = concat(wbenf, wr, y, n, b1(0))
                elif s.is_evict_M1:
                    s.cs1 = concat(wben0, rd, y, y, b1(1))
                elif s.is_write_hit_clean_M1:
                    s.cs1 = concat(wbenf, x, n, n, b1(0))
                elif (s.cachereq_type_M1 == INIT):
                    s.cs1 = concat(wben, wr, y, n, b1(0))
                elif ((~ s.hit_M1) and (~ s.ctrl_bit_dty_rd_M1)):
                    s.cs1 = concat(wben0, x, n, n, b1(0))
                elif ((~ s.hit_M1) and s.ctrl_bit_dty_rd_M1):
                    s.cs1 = concat(wben0, x, n, n, b1(0))
                elif (s.hit_M1 and (~ s.ctrl_bit_dty_rd_M1)):
                    if (s.cachereq_type_M1 == READ):
                        s.cs1 = concat(wben0, rd, y, n, b1(0))
                    elif (s.cachereq_type_M1 == WRITE):
                        s.cs1 = concat(wben, wr, y, n, b1(0))
                    else:
                        s.cs1 = concat(wben0, x, n, n, b1(0))
                elif (s.hit_M1 and s.ctrl_bit_dty_rd_M1):
        ...
        s.val_reg_M2 = RegEnRst(Bits1)(en=s.reg_en_M2, in_=s.val_M1, out=s.val_M2)
        s.is_evict_reg_M2 = RegEnRst(Bits1)(en=s.reg_en_M2, in_=s.is_evict_M1, out=s.is_evict_M2)
        s.hit_reg_M2 = RegEnRst(Bits1)(en=s.reg_en_M2, in_=s.hit_M1, out=s.hit_M2[0])
        s.is_refill_reg_M2 = RegRst(Bits1)(in_=s.is_refill_M1, out=s.is_refill_M2)
        s.is_write_refill_reg_M2 = RegRst(Bits1)(in_=s.is_write_refill_M1, out=s.is_write_refill_M2)
        s.is_write_hit_clean_reg_M2 = RegRst(Bits1)(in_=s.is_write_hit_clean_M1, out=s.is_write_hit_clean_M2)
        ...
        def comb_block_M2():
            ...
            if s.val_M2:
                if ((~ s.memreq_rdy) or (~ s.cacheresp_rdy)):
                    s.cs2 = concat(mxsel0, b1(0), y, READ, n, n)
                elif s.is_evict_M2:
                    s.cs2 = concat(mxsel0, b1(0), n, WRITE, y, n)
                elif s.is_refill_M2:
                    if (s.cachereq_type_M2 == READ):
                        s.cs2 = concat(s.msel, b1(1), n, READ, n, y)
                    elif (s.cachereq_type_M2 == WRITE):
                        s.cs2 = concat(mxsel0, b1(1), n, READ, n, y)
                elif s.is_write_hit_clean_M2:
                    s.cs2 = concat(mxsel0, b1(0), n, READ, n, n)
                elif (s.cachereq_type_M2 == INIT):
                    s.cs2 = concat(mxsel0, b1(0), n, READ, n, y)
                elif (s.cachereq_type_M2 == READ):
                    if s.hit_M2[0]:
                        s.cs2 = concat(s.msel, b1(0), n, READ, n, y)
                    elif (~ s.hit_M2[0]):
                        s.cs2 = concat(mxsel0, b1(0), n, READ, y, n)
                elif (s.cachereq_type_M2 == WRITE):
                    if s.is_write_refill_M2:
                        s.cs2 = concat(mxsel0, b1(1), n, READ, n, n)
                    elif s.hit_M2[0]:
                        s.cs2 = concat(mxsel0, b1(0), n, READ, n, y)
                    elif (~ s.hit_M2[0]):
                        s.cs2 = concat(mxsel0, b1(0), n, READ, y, n)
    ...
    def line_trace(s):
        ...
        if s.val_M2:
            if s.is_refill_M2:
                ...
                msg_M2 = 'wc'
            elif s.is_write_refill_M2:
                msg_M2 = 'wf'
            elif s.is_evict_M2:
                msg_M2 = 'ev'
            else:
                msg_M2 = types[s.cachereq_type_M2]
        msg_memresp = ('>' if s.memresp_en else ' ')
        msg_memreq = ('>' if s.memreq_en else ' ')
        states = ['Go', 'Rf', 'Ev', 'Wf', 'Wr']
        msg_state = states[s.curr_state]
        stage1 = ('{}|{}'.format(msg_memresp, msg_M0) if ((s.curr_state == STATE_REFILL) and s.memresp_en) else '  {}'.format(msg_M0))
