...
from NonBlockingCache.test_modules.TestCacheSink   import TestCacheSink
from NonBlockingCache.test_modules.TestCoherentMem import TestCoherentMemory as TestMemory
...
class TestHarness( Model ):
  def __init__( s, src_msgs, sink_msgs, stall_prob, latency,
    ...
    # Mem -> Cache
    s.memresp   = InValRdyBundle ( CoherentMemRespMsg16B )

    # Cache -> Proc
    s.cacheresp = OutValRdyBundle( MemRespMsg4B )

    # Cache -> Mem
    s.memreq    = OutValRdyBundle( CoherentMemReqMsg16B )

    # Mem -> Cache (fwdreq)

    s.fwdreq    = InValRdyBundle ( CoherentMemReqMsg16B )

    # Cache -> Mem (fwdresp)

    s.fwdresp   = OutValRdyBundle( CoherentMemRespMsg16B )
